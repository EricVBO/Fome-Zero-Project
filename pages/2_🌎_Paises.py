...
with st.container():
    ...
    df_aux = df1.loc[:, cols].groupby('Country').nunique().sort_values('Restaurant_ID', ascending = False).reset_index()
    fig = px.bar(df_aux, x= 'Country', y='Restaurant_ID', color='Country', text='Restaurant_ID', title='Quantidade de restaurantes cadastrados por país', labels={'Country': 'Países', 'Restaurant_ID': 'Quantidade de restaurantes'})
    st.plotly_chart(fig, use_container_width=True)
    st.markdown('''___''')

with st.container():
    cols = ['Country','City']
    df_aux = df1.loc[:, cols].groupby('Country').nunique().sort_values('City', ascending = False).reset_index()
    fig = px.bar(df_aux, x= 'Country', y='City', color='Country', text='City', title='Quantidade de cidades cadastradas por país', labels={'Country': 'Países', 'City': 'Quantidade de cidades cadastradas por país'})
    st.plotly_chart(fig, use_container_width=True)
    st.markdown('''___''')
...
with st.container():
    ...
    with col1:
        cols = ['Country', 'Votes']
        df_aux = df1.loc[:, cols].groupby('Country').mean().round(2).sort_values('Votes', ascending = False).reset_index()
        fig = px.bar(df_aux, x='Country', y='Votes', color='Country', text='Votes', title='Média de avaliações feitas por país', labels={'Country': 'Países', 'Votes': 'Quantidade de avaliações'})
        st.plotly_chart(fig, use_container_width=True)
    with col2:
        cols = ['Country', 'Average_Cost_for_two']
        df_aux = df1.loc[:, cols].groupby('Country').mean().round(2).reset_index()
        fig = px.bar(df_aux, x='Country', y='Average_Cost_for_two', color='Country', text='Average_Cost_for_two', title='Preço médio em BRL do prato para duas pessoas por país', labels={'Country': 'Países', 'Average_Cost_for_two': 'Preço para duas pessoas'})
        st.plotly_chart(fig, use_container_width=True)